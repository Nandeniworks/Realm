...
export const initSocketServer = (httpServer) => {
  ...
  io.use((socket, next) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET || 'realm_jwt_access_secret_key_2026_xYz', (err, decoded) => {
      if (err) {
        socket.user = { id: `guest_${socket.id.slice(-6)}`, username: `Guest_${socket.id.slice(-4)}` };
        console.log(`[Logging] Socket Authenticated as Guest: ${socket.user.username}`);
        return next();
      }
      socket.user = decoded;
      console.log(`[Logging] Socket Authenticated as User: ${socket.user.username}`);
      next();
    });
  });

  io.on('connection', (socket) => {
    console.log('[Logging] Socket Connected', socket.id);
